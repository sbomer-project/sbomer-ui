...
import { useEnhancementRun } from './useEnhancementRun';

const EnhancementRunPageContent: React.FunctionComponent = () => {
  const { id, runId } = useParams<{ id: string; runId: string }>();
  const [{ run, error, loading }] = useEnhancementRun(id!, runId!);

  useDocumentTitle(`SBOMer | Enhancement ${id} | Run ${runId}`);

  if (error) {
    return (
      <Stack gap={6}>
        <ErrorSection title="Could not load enhancement run" message={error.message} />
      </Stack>
    );
  }
    ...
    <Stack gap={7}>
      ...
      <StructuredListWrapper isCondensed>
        ...
        <StructuredListBody>
          ...
          <StructuredListRow>
            <StructuredListCell>Enhancement ID</StructuredListCell>
            <StructuredListCell>
              <Link to={`/enhancements/${run.enhancementId}`}>
                <pre>{run.enhancementId}</pre>
              </Link>
            </StructuredListCell>
          </StructuredListRow>
          <StructuredListRow>
...
export { EnhancementRunPageContent };
